...
def solve_wave_equation_3d(grid_size, time_steps, dt, dx, tol=1e-6):
    ...
    z = np.linspace(-2*np.pi, 2*np.pi, grid_size)
    X, Y, Z = np.meshgrid(x, y, z, indexing='ij')

    # 初始条件
    w[:, :, :, 0] = np.sin(X) * np.cos(Y) * np.sin(Z)
    w[:, :, :, 1] = w[:, :, :, 0]
        ...
        for i in range(2, grid_size - 2):
            for j in range(2, grid_size - 2):
                for k in range(2, grid_size - 2):
                    ...
                    # 更新网格点
                    new_w[i, j, k, t] = w_tttt - w_xxxx - w_xxtt - w_yyyy - w_yytt - w_zzzz - w_zztt
...
def create_animation(grid_size, solution):
    ...
    ani.save('wave_equation_3d.mp4', writer='ffmpeg', fps=20)
    plt.show()
